...
"""Modified PyTorch RoFormer model with SDPA speed up by ndat(at)utexas(dot)edu"""
    ...
    def forward(
        ...
        if not sdpa:
            ...
            context_layer = torch.nn.functional.scaled_dot_product_attention(query_layer, key_layer, \
                            value_layer, attn_mask = attention_mask, dropout_p = self.attention_probs_dropout_prob, scale = scale_factor)
